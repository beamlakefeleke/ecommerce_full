import 'package:ecommerce/common/widgets/corner_banner/banner.dart';
import 'package:ecommerce/common/widgets/corner_banner/corner_discount_tag.dart';
...
import 'package:ecommerce/common/widgets/not_available_widget.dart';
                ...
                child: Row(children: [
                  ...
                  Column(mainAxisAlignment: isStore ? MainAxisAlignment.center : MainAxisAlignment.spaceBetween, children: [
                    ...
                    fromCartSuggestion ? Container(
                      ...
                      bool isWished = isStore ? state.wishStoreIdList.contains(store!.id)
                          : state.wishItemIdList.contains(item!.id);
                      return InkWell(
                        onTap: !state.isRemoving ? () {
                          if(AuthHelper.isLoggedIn()) {
                            isWished ? getIt<FavouriteBloc>().add(FavouriteRemoved(id: isStore ? store!.id : item!.id, isStore: isStore))
                                : getIt<FavouriteBloc>().add(FavouriteAdded(item: item, store: store, isStore: isStore));
                          }else {
