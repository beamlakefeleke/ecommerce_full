...
import 'package:ecommerce/common/widgets/discount_tag.dart';
import 'package:ecommerce/common/widgets/quantity_button.dart';
      ...
      child: GetBuilder<ItemController>(builder: (itemController) {
        ...
        for (int index = 0; index < widget.item!.addOns!.length; index++) {
          if (itemController.addOnActiveList[index]) {
            ...
            addOnIdList.add(CartAddOn(id: widget.item!.addOns![index].id, quantity: itemController.addOnQtyList[index]));
                        ...
                        Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                          ...
                          Expanded(
                            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              ...
                              // FIXED: this InkWell + Padding + Text(storeName) was orphaned
                              // (stray `},` and a dangling `child:` with no parent widget) —
                              // reconstructed as a tappable store name. Confirm the route
                              // call matches your actual RouteHelper store-route method/args.
                              // InkWell(
                              //   onTap: () {
                              //     if (widget.item!.storeId != null) {
                              //       // TODO: verify against RouteHelper's actual store route signature
                              //       Get.toNamed(RouteHelper.getStoreRoute(widget.item!.storeId.toString()));
                              //     }
                              //   },
                              //   child: Padding(
                          ...
                          Column(mainAxisAlignment: MainAxisAlignment.spaceBetween, crossAxisAlignment: CrossAxisAlignment.end, children: [
                            ...
                            widget.isCampaign ? const SizedBox(height: 25) : BlocBuilder<FavouriteBloc, FavouriteState>(builder: (state) {
                              return InkWell(
                                onTap: () {
                                  if(AuthHelper.isLoggedIn()) {
                                    state.wishItemIdList.contains(widget.item!.id) ? getIt<FavouriteBloc>().add(FavouriteRemoved(id: widget.item!.id, isStore: false))
                                        : getIt<FavouriteBloc>().add(FavouriteAdded(item: widget.item, store: null, isStore: false));
                                  }else {
                                ...
                                child: Container(
                                  ...
                                  child: Icon(
                                    state.wishItemIdList.contains(widget.item!.id) ? Icons.favorite : Icons.favorite_border,
                                    color: state.wishItemIdList.contains(widget.item!.id) ? Theme.of(context).primaryColor
                  ...
                  child: Column(children: [
                    ...
                    SafeArea(
                      child: Row(children: [
                          ...
                          Expanded(child: BlocBuilder<CartBloc, CartState>(bloc: getIt<CartBloc>(),
                                    ...
                                    if(widget.isCampaign) {
                                      ...
                                      if (getIt<CartServiceInterface>().existAnotherStoreItem(
                                        ...
                                        Get.dialog(ConfirmationDialog(
                                          ...
                                          onYesPressed: () {
                                            ...
                                            getIt<CartBloc>().clearCartOnline().then((success) async {
                                              if(success) {
                                                await getIt<CartBloc>().addToCartOnline(onlineCart);
                                                Get.back();
                                                showCartSnackBar();
                                              }
                                            });
                                        ...
                                        if(widget.cart != null || itemController.cartIndex != -1){
                                          await getIt<CartBloc>().updateCartOnline(onlineCart).then((success) {
                                            if(success) {
                                              Get.back();
                                            }
                                          });
                                        } else {
                                          await getIt<CartBloc>().addToCartOnline(onlineCart).then((success) {
                                            if(success) {
                                              Get.back();
                                            }
                                          });
