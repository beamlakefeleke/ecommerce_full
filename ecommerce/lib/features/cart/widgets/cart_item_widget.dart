...
class CartItemWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    String? variationText = _setupVariationText(cart: CartModel.fromJson(cart.toJson()));
    String addOnText = _setupAddonsText(cart: CartModel.fromJson(cart.toJson())) ?? '';
          ...
          child: CustomInkWell(
            onTap: () {
              ResponsiveHelper.isMobile(context) ? showModalBottomSheet(
                ...
                builder: (con) => ItemBottomSheet(item: cart.item, cartIndex: cartIndex, cart: CartModel.fromJson(cart.toJson())),
              ) : showDialog(context: context, builder: (con) => Dialog(
                child: ItemBottomSheet(item: cart.item, cartIndex: cartIndex, cart: CartModel.fromJson(cart.toJson())),
              ));
            },
                  ...
                  BlocBuilder<CartBloc, CartState>(bloc: getIt<CartBloc>(),
                    builder: (context, cartState) {
                      return Row(children: [
                        ...
                        QuantityButton(
                          onTap: cartState.isLoading ? null : () {
                            getIt<CartBloc>().add(ForcefullySetModuleEvent(getIt<CartBloc>().state.cartList[0].item!.moduleId!));
                            getIt<CartBloc>().add(SetQuantityEvent(true, cartState.cartList[cartIndex], cartIndex, true));
                          },
                          isIncrement: true,
                          color: cartState.isLoading ? Theme.of(context).disabledColor : null,
                        ),
                      ]);
                    }
                  ),
  ...
  String? _setupVariationText({required Cart cart}) {
  ...
  String? _setupAddonsText({required Cart cart}) {
    String addOnText = '';
    int index0 = 0;
    List<int?> ids = [];
    List<int?> qtys = [];
