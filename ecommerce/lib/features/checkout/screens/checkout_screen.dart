...
import 'package:ecommerce/common/widgets/address_widget.dart';
...
import 'package:ecommerce/features/checkout/widgets/checkout_screen_shimmer_view.dart';
import 'package:ecommerce/features/checkout/widgets/payment_method_bottom_sheet.dart';
import 'package:get/get.dart';
import 'package:ecommerce/features/checkout/widgets/bottom_section.dart';
import 'package:ecommerce/features/checkout/widgets/top_section.dart';
...
class CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Future<void> initCall() async {
      ...
      getIt<CheckoutBloc>().add(const MostTippedAmountFetched());
      getIt<CheckoutBloc>().add(const PreferableTimeSet(time: ''));

      getIt<CheckoutBloc>().add(const OfflineMethodListFetched());

      if(getIt<CheckoutBloc>().state.isPartialPay){
      ...
      if(widget.storeId == null){
        ...
        widget.fromCart ? _cartList!.addAll(getIt<CartBloc>().state.cartList.cast<CartModel>()) : _cartList!.addAll(widget.cartList!);
        if(_cartList != null && _cartList!.isNotEmpty) {
          getIt<CheckoutBloc>().add(CheckoutInitialised(storeId: _cartList![0]!.item!.storeId));
        }
      }
      if(widget.storeId != null){
        getIt<CheckoutBloc>().add(CheckoutInitialised(storeId: widget.storeId));
        getIt<CheckoutBloc>().add(const PrescriptionImagePicked(isRemove: true));
        Get.find<CouponController>().removeCouponData(false);
      }
      _isWalletActive = Get.find<SplashController>().configModel!.customerWalletStatus == 1;
      
      int tipIndex = Get.find<CheckoutController>().getSharedPrefDmTipIndex().isNotEmpty ? int.parse(Get.find<CheckoutController>().getSharedPrefDmTipIndex()) : 0;
      getIt<CheckoutBloc>().add(TipUpdated(tipIndex: tipIndex));
      
      Get.find<CheckoutController>().tipController.text = getIt<CheckoutBloc>().state.selectedTips != -1 ? AppConstants.tips[getIt<CheckoutBloc>().state.selectedTips] : '';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: guestCheckoutPermission || AuthHelper.isLoggedIn() ? BlocBuilder<CheckoutBloc, CheckoutState>(builder: (context, checkoutState) {
        CheckoutController checkoutController = Get.find<CheckoutController>();

        List<DropdownItem<int>> addressList = _getDropdownAddressList(context: context, addressList: Get.find<AddressController>().addressList, store: checkoutState.store);
        address = _getAddressList(addressList: Get.find<AddressController>().addressList, store: checkoutState.store);

        bool todayClosed = false;
        bool tomorrowClosed = false;
        Pivot? moduleData = _getModuleData(store: checkoutState.store);
        _isCashOnDeliveryActive = _checkCODActive(store: checkoutState.store);
        _isDigitalPaymentActive = _checkDigitalPaymentActive(store: checkoutState.store);
        ...
        if(checkoutState.store != null) {
          todayClosed = checkoutController.isStoreClosed(true, checkoutState.store!.active!, checkoutState.store!.schedules);
          tomorrowClosed = checkoutController.isStoreClosed(false, checkoutState.store!.active!, checkoutState.store!.schedules);
          _taxPercent = checkoutState.store!.tax;
        }
        return GetBuilder<CouponController>(builder: (couponController) {
          ...
          double price = _calculatePrice(store: checkoutState.store, cartList: _cartList);
          double addOns = _calculateAddonsPrice(store: checkoutState.store, cartList: _cartList);
          double variations = _calculateVariationPrice(store: checkoutState.store, cartList: _cartList, calculateWithoutDiscount: true);
          double? discount = _calculateDiscount(
            store: checkoutState.store, cartList: _cartList, price: price, addOns: addOns,
          );
          ...
          double originalCharge = _calculateOriginalDeliveryCharge(
            store: checkoutState.store, address: AddressHelper.getUserAddressFromSharedPref()!,
            distance: checkoutState.distance, extraCharge: checkoutState.extraCharge,
          );
          double deliveryCharge = _calculateDeliveryCharge(
            store: checkoutState.store, address: AddressHelper.getUserAddressFromSharedPref()!, distance: checkoutState.distance,
            extraCharge: checkoutState.extraCharge, orderType: checkoutState.orderType!, orderAmount: orderAmount,
          );

          double total = _calculateTotal(
            subTotal: subTotal, deliveryCharge: deliveryCharge, discount: discount,
            couponDiscount: couponDiscount, taxIncluded: taxIncluded, tax: tax, orderType: checkoutState.orderType!,
            tips: checkoutState.tips, additionalCharge: additionalCharge,
          );

          if(widget.storeId != null){
            getIt<CheckoutBloc>().add(const PaymentMethodSet(index: 0));
          }
          getIt<CheckoutBloc>().add(TotalAmountSet(amount: total - (checkoutState.isPartialPay ? Get.find<ProfileController>().userInfoModel!.walletBalance! : 0)));

          return (checkoutState.distance != null && checkoutState.store != null) ? Column(
            children: [
                    ...
                    child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      Expanded(flex: 4, child: BottomSection(
                        ...
                        checkoutButton: _orderPlaceButton(
                          checkoutState, checkoutController, todayClosed, tomorrowClosed,
                  ...
                  children: [
                    Padding(
                      ...
                      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                        Text(
                          checkoutState.isPartialPay ? 'due_payment'.tr : 'total_amount'.tr,
                          style: robotoMedium.copyWith(fontSize: Dimensions.fontSizeLarge, color: Theme.of(context).primaryColor),
                        ),
                        PriceConverter.convertAnimationPrice(
                          checkoutState.viewTotalPrice,
                    ...
                    _orderPlaceButton(
                        checkoutState, checkoutController, todayClosed, tomorrowClosed, orderAmount, deliveryCharge, tax, discount, total, maxCodOrderAmount
                    ),
                  ],
                ),
              ),
  ...
  Widget _orderPlaceButton(CheckoutState checkoutState, CheckoutController checkoutController, bool todayClosed, bool tomorrowClosed,
    ...
    return Container(
      ...
      child: SafeArea(
        child: CustomButton(
          isLoading: checkoutState.orderStatus == CheckoutStatus.loading,
          buttonText: checkoutState.isPartialPay ? 'place_order'.tr : 'confirm_order'.tr,
          onPressed: checkoutState.acceptTerms ? () {
          bool isAvailable = true;
          DateTime scheduleStartDate = DateTime.now();
          DateTime scheduleEndDate = DateTime.now();
          bool isGuestLogIn = AuthHelper.isGuestLoggedIn();
          if(checkoutState.timeSlots == null || checkoutState.timeSlots!.isEmpty) {
            isAvailable = false;
          }else {
            DateTime date = checkoutState.selectedDateSlot == 0 ? DateTime.now() : DateTime.now().add(const Duration(days: 1));
            DateTime startTime = checkoutState.timeSlots![checkoutState.selectedTimeSlot].startTime!;
            DateTime endTime = checkoutState.timeSlots![checkoutState.selectedTimeSlot].endTime!;
            ...
            if(_cartList != null){
              for (CartModel? cart in _cartList!) {
                if (!DateConverter.isAvailable(
                  ...
                  time: checkoutState.store!.scheduleOrder! ? scheduleStartDate : null,
                ) && !DateConverter.isAvailable(
                  cart.item!.availableTimeStarts, cart.item!.availableTimeEnds,
                  time: checkoutState.store!.scheduleOrder! ? scheduleEndDate : null,
                )) {
                  isAvailable = false;
                  break;
                }
          ...
          if(isGuestLogIn && checkoutState.guestAddress == null && checkoutState.orderType != 'take_away') {
            showCustomSnackBar('please_setup_your_delivery_address_first'.tr);
          } else if(isGuestLogIn && checkoutState.orderType == 'take_away' && guestContactPersonNameController.text.isEmpty) {
            showCustomSnackBar('please_enter_contact_person_name'.tr);
          } else if(isGuestLogIn && checkoutState.orderType == 'take_away' && guestContactPersonNumberController.text.isEmpty) {
            showCustomSnackBar('please_enter_contact_person_number'.tr);
          }else if(isGuestLogIn && checkoutState.orderType == 'take_away' && guestEmailController.text.isEmpty) {
          ...
          }else if(checkoutState.paymentMethodIndex == -1) {
          ...
          } else if(orderAmount < checkoutState.store!.minimumOrder! && widget.storeId == null) {
            showCustomSnackBar('${'minimum_order_amount_is'.tr} ${checkoutState.store!.minimumOrder}');
          ...
          }else if((checkoutState.selectedDateSlot == 0 && todayClosed) || (checkoutState.selectedDateSlot == 1 && tomorrowClosed)) {
          ...
          }else if(checkoutState.paymentMethodIndex == 0 && _isCashOnDeliveryActive! && maxCodOrderAmount != null && maxCodOrderAmount != 0 && (total > maxCodOrderAmount) && widget.storeId == null){
            showCustomSnackBar('${'you_cant_order_more_then'.tr} ${PriceConverter.convertPrice(maxCodOrderAmount)} ${'in_cash_on_delivery'.tr}');
          }else if(checkoutState.paymentMethodIndex != 0 && widget.storeId != null){
            showCustomSnackBar('payment_method_is_not_available'.tr);
          }else if (checkoutState.timeSlots == null || checkoutState.timeSlots!.isEmpty) {
            if(checkoutState.store!.scheduleOrder!) {
          ...
          }else if (checkoutState.orderType != 'take_away' && checkoutState.distance == -1 && deliveryCharge == -1) {
            showCustomSnackBar('delivery_fee_not_set_yet'.tr);
          }else if (widget.storeId != null && checkoutState.pickedPrescriptions.isEmpty) {
            showCustomSnackBar('please_upload_your_prescription_images'.tr);
          }else if (!checkoutState.acceptTerms) {
            showCustomSnackBar('please_accept_privacy_policy_trams_conditions_refund_policy_first'.tr);
          }
          else {

            AddressModel? finalAddress = isGuestLogIn ? checkoutState.guestAddress : address[checkoutState.addressIndex!];

            if(isGuestLogIn && checkoutState.orderType == 'take_away') {
            ...
            if(widget.storeId == null){
              ...
              PlaceOrderBodyModel placeOrderBody = PlaceOrderBodyModel(
                cart: carts, couponDiscountAmount: Get.find<CouponController>().discount, distance: checkoutState.distance,
                scheduleAt: !checkoutState.store!.scheduleOrder! ? null : (checkoutState.selectedDateSlot == 0
                    && checkoutState.selectedTimeSlot == 0) ? null : DateConverter.dateToDateAndTime(scheduleEndDate),
                orderAmount: total, orderNote: checkoutController.noteController.text, orderType: checkoutState.orderType,
                paymentMethod: checkoutState.paymentMethodIndex == 0 ? 'cash_on_delivery'
                    : checkoutState.paymentMethodIndex == 1 ? 'wallet'
                    : checkoutState.paymentMethodIndex == 2 ? 'digital_payment' : 'offline_payment',
                ...
                chargePayer: null, dmTips: (checkoutState.orderType == 'take_away' || checkoutController.tipController.text == 'not_now') ? '' : checkoutController.tipController.text.trim(),
                cutlery: getIt<CartBloc>().state.addCutlery ? 1 : 0,
                unavailableItemNote: getIt<CartBloc>().state.notAvailableIndex != -1 ? getIt<CartBloc>().state.notAvailableList[getIt<CartBloc>().state.notAvailableIndex] : '',
                deliveryInstruction: checkoutState.selectedInstruction != -1 ? AppConstants.deliveryInstructionList[checkoutState.selectedInstruction] : '',
                partialPayment: checkoutState.isPartialPay ? 1 : 0, guestId: isGuestLogIn ? int.parse(AuthHelper.getGuestId()) : 0,
                isBuyNow: widget.fromCart ? 0 : 1, guestEmail: isGuestLogIn ? finalAddress.email : null,
              );

              if(checkoutState.paymentMethodIndex == 3){
                Get.toNamed(RouteHelper.getOfflinePaymentScreen(
                  placeOrderBody: placeOrderBody, zoneId: checkoutState.store!.zoneId!, total: checkoutState.viewTotalPrice,
                  maxCodOrderAmount: maxCodOrderAmount, fromCart: widget.fromCart, isCodActive: _isCashOnDeliveryActive, forParcel: false,
                ));
              } else {
                checkoutController.placeOrder(placeOrderBody, checkoutState.store!.zoneId, total, maxCodOrderAmount, widget.fromCart, _isCashOnDeliveryActive!);
              }
            }else{

              checkoutController.placePrescriptionOrder(widget.storeId, checkoutState.store!.zoneId, checkoutState.distance,
                  finalAddress!.address!, finalAddress.longitude!, finalAddress.latitude!, checkoutController.noteController.text,
                  checkoutState.pickedPrescriptions, (checkoutState.orderType == 'take_away' || checkoutController.tipController.text == 'not_now')
                      ? '' : checkoutController.tipController.text.trim(), checkoutState.selectedInstruction != -1
                      ? AppConstants.deliveryInstructionList[checkoutState.selectedInstruction] : '', 0, 0, widget.fromCart, _isCashOnDeliveryActive!
              );
            }
  ...
  double _calculateDeliveryCharge({required Store? store, required AddressModel address, required double? distance, required double? extraCharge, required double orderAmount, required String orderType}) {
    ...
    if (orderType == 'take_away' || (store != null && store.freeDelivery!)
        ...
        || Get.find<CouponController>().freeDelivery || (AuthHelper.isGuestLoggedIn() && (getIt<CheckoutBloc>().state.guestAddress == null && getIt<CheckoutBloc>().state.orderType != 'take_away'))) {
      deliveryCharge = 0;
    }
