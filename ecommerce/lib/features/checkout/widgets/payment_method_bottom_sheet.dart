...
import 'package:ecommerce/features/payment/widgets/offline_payment_button.dart';
import 'package:ecommerce/features/checkout/widgets/payment_button_new.dart';
...
class _PaymentMethodBottomSheetState extends State<PaymentMethodBottomSheet> {
  ...
  void initState() {
    ...
    if(!AuthHelper.isGuestLoggedIn()) {
      ...
      if(context.read<CheckoutBloc>().state.isPartialPay){
        notHideWallet = false;
                    ...
                    children: [
                      ...
                      Row(children: [
                        widget.isCashOnDeliveryActive && notHideCod ? Expanded(
                          child: PaymentButtonNew(
                            ...
                            isSelected: state.paymentMethodIndex == 0,
                            onTap: () {
                              context.read<CheckoutBloc>().add(const PaymentMethodSet(index: 0));
                            },
                          ),
                        ) : const SizedBox(),
                        ...
                        widget.storeId == null && widget.isWalletActive && notHideWallet && isLoggedIn ? Expanded(
                          child: PaymentButtonNew(
                            ...
                            isSelected: state.paymentMethodIndex == 1,
                            onTap: () {
                              if(canSelectWallet) {
                                context.read<CheckoutBloc>().add(const PaymentMethodSet(index: 1));
                              } else if(state.isPartialPay){
                      ...
                      widget.storeId == null && widget.isDigitalPaymentActive && notHideDigital ? ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            bool isSelected = state.paymentMethodIndex == 2 && Get.find<SplashController>().configModel!.activePaymentMethodList![index].getWay! == state.digitalPaymentName;
                          return InkWell(
                            onTap: (){
                              context.read<CheckoutBloc>().add(const PaymentMethodSet(index: 2));
                              context.read<CheckoutBloc>().add(DigitalPaymentNameChanged(name: Get.find<SplashController>().configModel!.activePaymentMethodList![index].getWay!));
                            },
                            child: Container(
                              decoration: BoxDecoration(
                      ...
                      OfflinePaymentButton(
                        isSelected: state.paymentMethodIndex == 3,
                        offlineMethodList: state.offlineMethodList,
                        isOfflinePaymentActive: widget.isOfflinePaymentActive,
                        onTap: () {
                          context.read<CheckoutBloc>().add(const PaymentMethodSet(index: 3));
                        },
                        selectedOfflineBankIndex: state.selectedOfflineBankIndex,
                        onSelectOfflineBank: (index) => context.read<CheckoutBloc>().add(OfflineBankSelected(index: index)),
