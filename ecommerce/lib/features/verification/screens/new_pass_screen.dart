    ...
    if (password.isEmpty) {
      ...
      if(widget.fromPasswordChange) {
        ...
        UserInfoModel updatedUser = UserInfoModel(
          ...
          image: user.image, phone: user.phone, createdAt: user.createdAt,
          password: password, orderCount: user.orderCount, memberSinceDays: user.memberSinceDays,
          walletBalance: user.walletBalance, loyaltyPoint: user.loyaltyPoint,
          refCode: user.refCode, socialId: user.socialId, userInfo: user.userInfo,
        );
        Get.find<ProfileController>().changePassword(updatedUser).then((response) {
