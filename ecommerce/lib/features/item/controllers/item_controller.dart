...
import 'package:ecommerce/features/item/domain/models/basic_medicine_model.dart';
import 'package:ecommerce/features/cart/domain/models/cart_model.dart';
import 'package:ecommerce/features/item/domain/models/common_condition_model.dart';
...
import 'package:ecommerce/common/widgets/item_bottom_sheet.dart';
...
class ItemController extends GetxController implements GetxService {
  ...
  int setExistInCart(Item? item, {bool notify = false}) {
    ...
    if(ModuleHelper.getModuleConfig(ModuleHelper.getModule() != null ? ModuleHelper.getModule()!.moduleType : ModuleHelper.getCacheModule()!.moduleType).newVariation!) {
      ...
      _cartIndex = getIt<CartServiceInterface>().isExistInCart(getIt<CartBloc>().state.cartList, item.id, variationType, false, null);
    }

    if(_cartIndex != -1) {
      _quantity = getIt<CartBloc>().state.cartList[_cartIndex].quantity;
      _addOnActiveList = itemServiceInterface.initializeCartAddonActiveList(getIt<CartBloc>().state.cartList[_cartIndex].addOnIds, item.addOns);
      _addOnQtyList = itemServiceInterface.initializeCartAddonsQtyList(getIt<CartBloc>().state.cartList[_cartIndex].addOnIds, item.addOns);
    }
    if(notify) {
      update();
    }
  ...
  void itemDirectlyAddToCart(Item? item, BuildContext context, {bool inStore = false, bool isCampaign = false}) {
    ...
    if (((item!.foodVariations != null && item.foodVariations!.isEmpty) && item.moduleType == AppConstants.food) || (item.variations != null && item.variations!.isEmpty && item.moduleType != AppConstants.food)) {
      ...
      if(Get.find<SplashController>().configModel!.moduleConfig!.module!.stock! && item.stock! <= 0){
      ...
      else if (getIt<CartServiceInterface>().existAnotherStoreItem(cartModel.item!.storeId, ModuleHelper.getModule() != null
        ...
        getIt<CartBloc>().addToCartOnline(onlineCart);
        showCartSnackBar();
      }
