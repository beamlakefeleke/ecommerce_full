...
import 'package:ecommerce/common/widgets/cart_snackbar.dart';
...
import 'package:ecommerce/features/item/widgets/details_app_bar_widget.dart';
import 'package:ecommerce/features/item/widgets/details_web_view_widget.dart';
import 'package:ecommerce/features/item/widgets/item_image_view_widget.dart';
import 'package:ecommerce/features/item/widgets/item_title_view_widget.dart';
...
class _ItemDetailsScreenState extends State<ItemDetailsScreen> {
      ...
      builder: (cartController) {
        return GetBuilder<ItemController>(
          builder: (itemController) {
            ...
            int? cartId = getIt<CartServiceInterface>().getCartId(itemController.cartIndex, cartState.cartList);
            if(itemController.item != null && itemController.variationIndex != null){
              ...
              for (int index = 0; index < itemController.item!.addOns!.length; index++) {
                if (itemController.addOnActiveList[index]) {
                  ...
                  addOnIdList.add(CartAddOn(id: itemController.item!.addOns![index].id, quantity: itemController.addOnQtyList[index]));
              ...
              cart = OnlineCart(
                  ...
                  itemController.cartIndex != -1 ? cartState.cartList[itemController.cartIndex].quantity
                            ...
                            return Row(children: [
                              ...
                              Container(
                                ...
                                child: Row(children: [
                                  InkWell(
                                    onTap: cartState.isLoading ? null : () {
                                      if(itemController.cartIndex != -1) {
                                        if(cartState.cartList[itemController.cartIndex].quantity! > 1) {
                                  ...
                                  !cartState.isLoading ? Text(
                                    itemController.cartIndex != -1 ? cartState.cartList[itemController.cartIndex].quantity.toString()
                                        : itemController.quantity.toString(),
                                    style:robotoMedium.copyWith(fontSize: Dimensions.fontSizeExtraLarge),
                                  ) : const SizedBox(height: 20, width: 20, child: CircularProgressIndicator()),

                                  InkWell(
                                    onTap: cartState.isLoading ? null : () => itemController.cartIndex != -1
                        ...
                        Row(children: [
                          ...
                          Text(
                            PriceConverter.convertPrice(itemController.cartIndex != -1
                                ? _getItemDetailsDiscountPrice(cart: getIt<CartBloc>().state.cartList[itemController.cartIndex])
                        ...
                        onPressed: (!Get.find<SplashController>().configModel!.moduleConfig!.module!.stock! || stock! > 0) ?  () async {
                          if(!Get.find<SplashController>().configModel!.moduleConfig!.module!.stock! || stock! > 0) {
                            if(itemController.item!.availableDateStarts != null) {
                              ...
                              if (getIt<CartServiceInterface>().existAnotherStoreItem(cartModel!.item!.storeId, Get.find<SplashController>().module == null ? Get.find<SplashController>().cacheModule!.id : Get.find<SplashController>().module!.id, getIt<CartBloc>().state.cartList), cartState.cartList) {
                                Get.dialog(ConfirmationDialog(
                                  ...
                                  onYesPressed: () {
                                    Get.back();
                                    getIt<CartBloc>().clearCartOnline().then((success) async {
                                      if(success) {
                                        await getIt<CartBloc>().addToCartOnline(cart!);
                                ...
                                if(itemController.cartIndex == -1) {
                                  await getIt<CartBloc>().addToCartOnline(cart!).then((success) {
                                    if(success){
                                      itemController.setExistInCart(widget.item);
                                      showCartSnackBar();
                                  ...
                                  await getIt<CartBloc>().updateCartOnline(cart!).then((success) {
                                    if(success) {
                                      showCartSnackBar();
                                      _key.currentState!.shake();
                                    }
  ...
  List<int?> _getSelectedAddonIds({required List<CartAddOn> addOnIdList }) {
  ...
  List<int?> _getSelectedAddonQtnList({required List<CartAddOn> addOnIdList }) {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          color: (quantity! == 1 && !isIncrement) || cartState.isLoading ? Theme.of(context).disabledColor : Theme.of(context).primaryColor,
        ),
        child: Center(
          child: Icon(
