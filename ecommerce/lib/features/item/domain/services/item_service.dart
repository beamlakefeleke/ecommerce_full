...
class ItemService implements ItemServiceInterface {
  ...
  @override
  List<bool> initializeCartAddonActiveList(List<CartAddOn>? addOnIds, List<AddOns>? addOns) {
    List<bool> addOnActiveList = [];
    if(addOnIds != null) {
      for (var addOnId in addOnIds) {
        for (var addOn in addOns!) {
          if (addOnId.id == addOn.id) {
  ...
  @override
  List<int?> initializeCartAddonsQtyList(List<CartAddOn>? addOnIds, List<AddOns>? addOns) {
    List<int?> addOnQtyList = [];
    if(addOnIds != null) {
      for (var addOnId in addOnIds) {
        for (var addOn in addOns!) {
          if (addOnId.id == addOn.id) {
            addOnQtyList.add(addOnId.quantity);
          }
        }
      }
    }
